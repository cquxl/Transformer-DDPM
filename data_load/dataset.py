...
class LorenzSeqDataset(Dataset):
    """
       1. 该类实现异常检测Lorenz数据集的Dataset构造，训练集和测试集
    ...
    def __init__(self, data_path, win_size, slide_step, mode='train', transform=True):
        ...
        anomaly_ratio = sum(self.test_labels) / (len(self.train) + len(self.val))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            return np.float32(self.val[
                ...
                self.test_labels[
                index // self.slide_step * self.win_size:index // self.slide_step * self.win_size + self.win_size])
...
def get_data_loader(data_path, batch_size, win_size=100, slide_step=100, mode='train',
    ...
    if (dataset == 'SMD'):
        ...
        dataset = LorenzSeqDataset(data_path, win_size, slide_step, mode, transform)
...
if __name__ == '__main__':
    print('--------------MSL数据集------------------')
    lorenz_dataset = LorenzSeqDataset(data_path='../data/Lorenz', win_size=100, slide_step=100, mode='train', transform=True)
    print(lorenz_dataset[0])
